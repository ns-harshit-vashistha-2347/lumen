...
export function Sources({ sources }: { sources: SourceChunk[] }) {
  ...
  // Heatmap filter: when set, only show citations from this doc key.
  ...
  const visibleSources = useMemo(
    () => (filterKey ? sources.filter((s) => keyOf(s) === filterKey) : sources),
    [sources, filterKey]
  );
  ...
  return (
    <>
      ...
      />
      <div className="mt-2 rounded-md border border-chrome-border bg-bg/50">
        <button
          onClick={() => setOpen((v) => !v)}
        ...
        >
          <span className="flex items-center gap-2">
            ...
            <span>
              ...
              {filterKey ? `/${sources.length}` : ""} chunk
              {(filterKey ? visibleSources.length : sources.length) === 1 ? "" : "s"}
            </span>
            <span className="text-ink-faint">·</span>
